...
glmFirstPass(GLMmodel* model, FILE* file) 
{
  ...
  group = model->groups;
  while(group) {
      group->triangles = (GLuint*)malloc(sizeof(GLuint) * (group->numtriangles + 300));//!!!!!!!!!!
      group->numtriangles = 0;
      group = group->next;
  }
}
...
glmSecondPass(GLMmodel* model, FILE* file) 
{
    ...
    allocated arrays */
    numvertices = numnormals = numtexcoords = 1;//index is start from 1
    numtriangles = 0;
    material = 0;
    while(fscanf(file, "%s", buf) != EOF) {
        switch(buf[0]) {
 ...
 */
GLfloat
glmUnitize(GLMmodel* model)// this is usually used for tool models
{
    GLuint i;
    ...
    for (i = 1; i <= model->numvertices; i++) {
        model->vertices[3 * i + 0] -= cx;//7.24
        model->vertices[3 * i + 1] -= cy;//8.56
        model->vertices[3 * i + 2] -= cz;//4.98
        model->vertices[3 * i + 0] *= scale;//0.03
...
void 
glmUnitize0(GLMmodel* model)//added by me for special use, it can fix the intestine and background model with a special position and scaling
{
    GLuint i;
    
    /* translate around center then scale */
    for (i = 1; i <= model->numvertices; i++) {
        model->vertices[3 * i + 0] -= 7.24;//
        model->vertices[3 * i + 1] -= 8.56;//
        model->vertices[3 * i + 2] -= 4.98;//
        model->vertices[3 * i + 0] *= 0.03;//
        model->vertices[3 * i + 1] *= 0.03;
        model->vertices[3 * i + 2] *= 0.03;
    }  
}
...
glmReadOBJ(const char* filename)
{
    ...
    model->vertices = (GLfloat*)malloc(sizeof(GLfloat) *
        3 * (model->numvertices + 200));//model->numvertices + 1, these numbers (200,300) are added for mesh subdivision cutting, under development
    model->triangles = (GLMtriangle*)malloc(sizeof(GLMtriangle) *
        (model->numtriangles + 300));//model->numtriangles
    if (model->numnormals) {
        model->normals = (GLfloat*)malloc(sizeof(GLfloat) *
            3 * (model->numnormals + 200));//model->numnormals + 1
    }
    if (model->numtexcoords) {
        model->texcoords = (GLfloat*)malloc(sizeof(GLfloat) *
            2 * (model->numtexcoords + 200));//model->numtexcoords + 1
    }
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    else if (mode & GLM_LINES)
	{
       glPolygonMode(GL_FRONT,GL_LINE);
  	   glPolygonMode(GL_BACK,GL_LINE);

       glLineWidth(1.0);/////force line
