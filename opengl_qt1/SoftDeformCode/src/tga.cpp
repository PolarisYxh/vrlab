...
TextureImage BGtextures[3];									// Storage For One Texture
...
// an image
tgaInfo * tgaLoad(char *filename, int normal) {
...
// check for errors when reading the pixels
	...
	fclose(file);
    buildTGATexture(&BGtextures[normal], filename); // Build the texture from tga. 
	info->status = TGA_OK;
	return(info);
}		
...
void changeTexture(int normal)//normal = 0 is for intestine texture, = 1 is for the background texture
{
	GLuint		type=GL_RGBA;

	glGenTextures(1, &BGtextures[normal].texID);					// Generate OpenGL texture IDs

	glBindTexture(GL_TEXTURE_2D, BGtextures[normal].texID);			// Bind Our Texture
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);	// Linear Filtered
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);	// Linear Filtered

	if (BGtextures[normal].bpp==24)									// Was The TGA 24 Bits
	{
		type=GL_RGB;										// If So Set The 'type' To GL_RGB
	}

	glTexImage2D(GL_TEXTURE_2D, 0, type, BGtextures[normal].width, BGtextures[normal].height, 0, type, GL_UNSIGNED_BYTE, BGtextures[normal].imageData);
}
