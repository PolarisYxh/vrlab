...
#include <pcl/visualization/pcl_visualizer.h>
 int main( int argc, char** argv )
 {
	 ...
	 std::string rootpath = "D:/my/sunxun/test3";
	 std::string depthpath = rootpath + "/depth";
	 std::string labelpath = rootpath + "/label";
	 std::string posepath=
	 ...
	 for ( int i=0; i<num; i++ )
	 {
		 string labelpath="./label/" //图像文件格式 ./color/1.png
	...
	for(int i=0;i<depthImgs[0].rows;i++)
	{
		   for(int j=0;j<depthImgs[0].cols;j++)
		{
		  if((unsigned int)depthImgs[0].at<unsigned short>(i,j)!=0)
		cout<<(unsigned int)depthImgs[0].ptr<unsigned short> ( i )[j]<<" ";//unsigned short 16bit
		}
	}
	 // 计算点云并拼接
	 // 相机内参
	 ...
	 double depthScale = 1000;//真实的kinect返回的数据是mm单位的，所以转为m，
	   /* double cx = 325.5;
		double cy = 253.5;
		double fx = 518.0;
		double fy = 519.0;
